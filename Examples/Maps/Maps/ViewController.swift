...
class FloatingPanelMiddlewareController {
    ...
    var backgroundColor: UIColor = .clear{
        didSet {
            if #available(iOS 10.0, *) {
                asFloatingPanel().surfaceView.backgroundColor = backgroundColor
            } else {
    ...
    var cornerRadius: CGFloat = 9.0 {
        didSet {
            if #available(iOS 10.0, *) {
                asFloatingPanel().surfaceView.cornerRadius = cornerRadius
            } else {
                // Fallback on earlier versions
            }
        }
    ...
    var shadowHidden: Bool = false {
        didSet {
            if #available(iOS 10.0, *) {
                asFloatingPanel().surfaceView.shadowHidden = shadowHidden
            } else {
                // Fallback on earlier versions
            }
        }
    ...
    @available(iOS 10.0, *)
    func asFloatingPanel() -> FloatingPanelController {
    ...
    func addPanel(toParent parent: UIViewController, belowView: UIView? = nil, animated: Bool = false) {
        if #available(iOS 10.0, *) {
            asFloatingPanel().addPanel(toParent: parent, belowView: belowView, animated: animated)
        } else {
            // Fallback on earlier versions
        }
    }
    ...
    func move(to: FloatingPanelPosition, animated: Bool, completion: (() -> Void)? = nil) {
        if #available(iOS 10.0, *) {
            asFloatingPanel().move(to: to, animated: animated, completion: completion)
        } else {
            // Fallback on earlier versions
        }
    }
    ...
    func set(contentViewController: UIViewController) {
        if #available(iOS 10.0, *) {
            asFloatingPanel().set(contentViewController: contentViewController)
        } else {
            // Fallback on earlier versions
        }
    }
    ...
    func track(scrollView: UIScrollView) {
        if #available(iOS 10.0, *) {
            asFloatingPanel().track(scrollView: scrollView)
        } else {
            // Fallback on earlier versions
        }
    }
...
@available(iOS 10.0, *)
extension FloatingPanelMiddlewareController: FloatingPanelControllerDelegate {
    func floatingPanel(_ vc: FloatingPanelController, layoutFor newCollection: UITraitCollection) -> FloatingPanelLayout? {
        switch newCollection.verticalSizeClass {
        case .compact:
            fpc.surfaceView.borderWidth = 1.0 / traitCollection.displayScale
